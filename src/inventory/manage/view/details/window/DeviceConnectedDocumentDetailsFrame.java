...
import inventory.manage.view.util.PMCustomTextFieldWithDictionary;
...
public class DeviceConnectedDocumentDetailsFrame extends JDialog {
	...
	private PMCustomTextFieldWithDictionary<Document> documentField;
	...
	public DeviceConnectedDocumentDetailsFrame(Controller controller) {
		super(controller.getMainFrame(), "Panel", true);
		this.controller = controller;
		this.controller.setDeviceConnectedDocumentDetailsFrame(this);
	}
	...
	private JPanel createHeaderPanel() {
		...
		JLabel titleHeaderLabel = new JLabel("Powiązanie dokumentu");
	...
	public JPanel createDetailsPanel() {
		...
		deviceField = new PMCustomTextFieldWithDictionary<Device>(false);
		detailsPanel.add(deviceField, "cell 2 1 4 1,grow");
		
		JLabel documentLabel= new JLabel("Dokument:");
		documentLabel.setFont(new Font("Tahoma", Font.PLAIN, 13));
		detailsPanel.add(documentLabel, "cell 1 2,alignx left");
		
		documentField = new PMCustomTextFieldWithDictionary<Document>(isEditable());
		detailsPanel.add(documentField, "cell 2 2 4 1,grow");
	...
	public void createEventListeners() {
		...
		documentField.getDictionaryButton().removeActionListener(documentField.getDictionaryButton().getActionListeners()[0]);
		documentField.getDictionaryButton().addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				getController().getManagerDAO().getDocumentServiceDAO().openDocumentDictionaryWindowAndAddItem();
			}
		});
	}
	...
	public void assignDocumentToDevice() {
		
		Document document = documentField.getItem();
		ConnectedDocument connectedDocument = new ConnectedDocument();
		connectedDocument.setDocumentID(document.getDocumentID());
		connectedDocument.setDeviceID(deviceField.getItem().getDeviceID());
		
		document.setConnectedDocument(connectedDocument);
		if(getController().getManagerDAO().getDocumentServiceDAO().assignDocumentToDevice(document)) dispose();
	}
	...
	private boolean validateFields() {
		...
		errors.add(documentField.isEmpty());
		
		return !errors.contains(true);
	}

	private String getCorrectTitle() {
		return (isEditable()) ? "Przypisanie dokumentu" : "Przypisanie dokumentu";
	}
	...
	public PMCustomTextFieldWithDictionary<Document> getDocumentField() {
